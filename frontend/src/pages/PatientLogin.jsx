    ...
    <div className="container auth-page">
      <div className="auth-card">
        <div className="card">
          ...
          <h1 className="page-title">{t('auth.patientLoginTitle')}</h1>
          <p className="page-subtitle">{t('auth.patientLoginSub')}</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="btn btn-patient" style={{ width: '100%' }} disabled={loading}>
              {loading ? t('auth.signingIn') : t('auth.signInPatient')}
            </button>
          </form>
          <p className="auth-footer">
            {t('auth.noAccount')} <Link to="/register/patient">{t('auth.registerPatient')}</Link>
          </p>
        </div>
      </div>
    </div>
